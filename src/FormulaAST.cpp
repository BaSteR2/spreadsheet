...
public:
    ...
    virtual double Evaluate(const std::function<double(const Position&)>& GetValueFromSheet) const = 0;
...
public:
    ...
    double Evaluate(const std::function<double(const Position&)>& GetValueFromSheet) const override { 
        double solution = 0.0;
        switch (type_) {
            case Add :
                solution = lhs_->Evaluate(GetValueFromSheet) + rhs_->Evaluate(GetValueFromSheet);
...
public:
    ...
    double Evaluate(const std::function<double(const Position&)>& GetValueFromSheet) const override { 
        ...
        switch (type_) {
            case UnaryPlus :
                solution = operand_->Evaluate(GetValueFromSheet);
                break;
            case UnaryMinus :
                solution = operand_->Evaluate(GetValueFromSheet) * (-1);   
                break;
        }
        return solution;
    }
...
public:
    ...
    double Evaluate(const std::function<double(const Position&)>& GetValueFromSheet) const override { 
        return GetValueFromSheet(*cell_);
    }
...
double FormulaAST::Execute(const std::function<double(const Position&)>& GetValueFromSheet) const {
    return root_expr_->Evaluate(GetValueFromSheet); 
}
